...
if(isset($_POST['changes']))
{
    ...
    $empID = trim($_POST['empIDS_b']);
    $dtrID = trim($_POST['dtrID_b']);
    $off_status = trim($_POST['off_status']);
    ...
    if($count > 0){
        ...
        $sql_off_approved = "UPDATE `dtrs` SET `off_status` = '$off_status', `off_approved` = '$approved_by'
                    WHERE `dtrs`.`ID` = '$dtrID'";
    ...
    // }
        ...
        if($con->query($sql_off_approved) or die ($con->error))
        {
